    ...
    swal(`You beat ${this.master.name}!`).then(async() => {
      if (currentRank == this.master.rank) {
        if(this.master.gm){
          ...
          swal(`You beat level ${currentRank}! Well done!`).then(() => { this.router.navigate(['/dojo']) });
        }else{
        ...
        if(nextMaster) { //current rank is an even number, +1 makes it odd, only gm-s have odd ranks
          swal(`You are now ready to face ${nextMaster.name}`).then(() => { this.router.navigate(['/dojo']) });
        }else{
          swal(`A new lesson have opened up.`).then(() => { this.router.navigate(['/dojo']) });
        }
      }
      }
